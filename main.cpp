...
int main(){
	...
	int pilihan;
	string username; //variabel penyimpan nama user
	...
	if(pilihan == 1){
		...
		while(temp == InfoHead(Q)){
			...
			// PlaySoundmainn(TEXT("sound\\op.wav"),NULL,SND_ASYNC|SND_LOOP);	//playsound
			...
			SetConsoleTextAttribute(h,FOREGROUND_BLUE | FOREGROUND_INTENSITY);
			cout << "Berapa jumlah Bintang di scene " << no << " ?" << endl;
			SetConsoleTextAttribute(h,FOREGROUND_RED | FOREGROUND_INTENSITY);
			cin >> temp;
			system("cls");
		}
